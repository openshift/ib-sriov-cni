...
"${OCI_RUNTIME}" build -t "${IMAGE_UNDER_TEST}" -f ./Dockerfile.rhel7 .
...
if ! "${OCI_RUNTIME}"  run -e CNI_COMMAND=VERSION -v "${OUT_RHEL8}/ib-sriov:/ib-sriov" registry.access.redhat.com/ubi8/ubi /ib-sriov ; then
    echo "RHEL8 binary does not run on ubi8 image"
    exit 1
fi

if ! "${OCI_RUNTIME}"  run -e CNI_COMMAND=VERSION -v "${OUT_RHEL9}/ib-sriov:/ib-sriov" registry.access.redhat.com/ubi9/ubi /ib-sriov ; then
    echo "RHEL9 binary does not run on ubi9 image"
    exit 1
fi

cmp "${OUT_DEFAULT}/ib-sriov" "${OUT_RHEL9}/ib-sriov" || { echo "default and RHEL9 binaries differ"; exit 1; }
